...
func (pki *PKI) loadCertForIssuer(issuerRef string) (*x509.Certificate, error) {
	secret, err := pki.vault.Logical().Read(fmt.Sprintf("/%s/issuer/%s/json", pki.path, issuerRef))
	if err != nil {
		return nil, fmt.Errorf("error finding CA cert at /%s/issuer/%s/json: %w", pki.path, issuerRef, err)
	}
	...
	err = mapstructure.Decode(secret.Data, &secretCert)
	if err != nil {
		slog.Error("Failed to decode secret", "pki", pki.path, "issuer", issuerRef, "error", err)
		return nil, fmt.Errorf("CA cert data missing or invalid for issuer %s", issuerRef)
	}

	block, _ := pem.Decode([]byte(secretCert.Certificate))
	if block == nil {
		slog.Error("Failed to decode PEM block", "pki", pki.path, "issuer", issuerRef)
		return nil, fmt.Errorf("CA cert data missing or invalid for issuer %s", issuerRef)
	}

	cert, err := x509.ParseCertificate(block.Bytes)
	if err != nil {
		slog.Error("Failed to load certificate", "pki", pki.path, "issuer", issuerRef, "error", err)
		return nil, fmt.Errorf("Failed to load certificate for %s", issuerRef)
	}
